...
const ChatGPTButton = ({ companyName, town, onUpdateLead, leadId }) => {
  ...
  const handleResearch = async () => {
    if (!companyName || !town) {
      setError("Company name and town are required");
      return;
    }
  ...
  const handleUpdateLead = async () => {
    if (!results || !onUpdateLead || !leadId) return;

    try {
      const updates = {};
      if (results.email) updates.email = results.email;
      if (results.website) updates.website = results.website;
      if (results.strategy.priority === "high") {
        updates.status = "High Priority";
      }
      if (results.analysis) {
        updates.notes = results.analysis;
      }
  ...
  return (
    <>
      <StyledButton onClick={() => setIsOpen(true)}>Research</StyledButton>

      {isOpen && (
        <StyledModal onClick={() => setIsOpen(false)}>
          <StyledModalContent onClick={(e) => e.stopPropagation()}>
            <StyledCloseButton onClick={() => setIsOpen(false)}>×</StyledCloseButton>
            <StyledTitle>Company Research: {companyName}</StyledTitle>

            {!results && !loading && !error && (
              <div>
                <p style={{ marginBottom: "1.5rem", color: colours.darkGrey }}>
                  Research company information using AI to find email and website.
                </p>
                <StyledActionButton $variant="primary" onClick={handleResearch}>
                  Start Research
                </StyledActionButton>
              </div>
            )}
            ...
            {error && (
              <div style={{ color: "#E51F28", marginBottom: "1rem" }}>
                Error: {error}
              </div>
            )}
            ...
            {results && (
              <>
                <StyledResultSection>
                  ...
                  {results.email ? (
                    <div style={{ display: "flex", alignItems: "center" }}>
                      ...
                      <StyledCopyButton onClick={() => handleCopy(results.email)}>
                        Copy
                      </StyledCopyButton>
                    </div>
                  ) : (
                ...
                <StyledActionButtons>
                  <StyledActionButton $variant="primary" onClick={handleUpdateLead}>
                    Update Lead
                  </StyledActionButton>
                  <StyledActionButton onClick={() => setIsOpen(false)}>Close</StyledActionButton>
                </StyledActionButtons>
              </>
            )}
          </StyledModalContent>
