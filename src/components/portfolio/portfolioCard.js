...
const PortfolioCard = () => {
    ...
    const fetchPortfolio = async (page) => {
        try {
            let queryPrams = `populate=*&pagination[page]=${page}&pagination[pageSize]=3`
                            ...
                            <div className="case-study u-pad-bottom-x3 u-pad-bottom-x4@m u-pad-bottom-x5@l">
                                <div className="o-wrap">
                                    <div className="c-card +default">
                                        <Link href={`/portfolio/${item?.id}`} className="c-media-box +offset">
                                            <img src={`${process.env.NEXT_PUBLIC_STRAPI_BASE_URL}${item?.attributes?.image?.data?.attributes?.url}`} />
                                        </Link>
